...
public class GetBusLocationTask extends TimerTask 
{
	...
	JSONParser j;
	public int BUS_DISTANCE_LIMIT = 5000;
	public int API_DISTANCE_LIMIT = 30000;
	
	Location SanJose = new Location("Metafora");
	...
	public GetBusLocationTask(GoogleMap map, Context context)
	{
		...
		SanJose.setLatitude(37.324049);
		SanJose.setLongitude(-121.919034);
	}
	...
	{	
		...
		protected ArrayList<MarkerOptions> doInBackground(String... params) 
		{
			buslist.clear();
			String result;
			if(gps.location.distanceTo(SanJose) < API_DISTANCE_LIMIT && gps.canGetLocation)
			{
				...
				try
				{
					...
					for(int c = 0; c < buses.length(); c ++)
					{
						...
						if(gps.location.distanceTo(loc) < BUS_DISTANCE_LIMIT && gps.canGetLocation)
						{
